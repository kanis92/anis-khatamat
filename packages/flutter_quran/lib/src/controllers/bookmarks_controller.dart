...
class BookmarksCubit extends Cubit<List<Bookmark>> {
  ...
  /// Affiche temporairement le résultat de navigation dans le Mushaf.
  ///
  /// Le timer appartient au Cubit qui porte l'état qu'il modifiera. Sa
  /// fermeture annule donc la source asynchrone avant de fermer le flux :
  /// aucune callback externe ne conserve une référence vers un Cubit fermé.
  void highlightSearchResult({
    ...
    saveBookmark(
      ayahId: ayahId,
      page: page,
      bookmarkId: searchBookmark.id,
    );
    _searchHighlightTimer = Timer(duration, () {
      _searchHighlightTimer = null;
      removeBookmark(searchBookmark.id);
    });
  }
