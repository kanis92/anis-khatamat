...
class QuranCubit extends Cubit<List<QuranPage>> {
  ...
  /// Un seul chargement peut posséder les structures et l'émission à la fois.
  ///
  /// Cette opération est aussi attendue par [close] : le Cubit n'est fermé
  /// qu'après la fin de l'I/O et de l'émission qu'il a lui-même démarrées.
  Future<void> loadQuran({quranPages = QuranRepository.hafsPagesNumber}) {
    return _loadOperation ??=
        _loadQuran(quranPages: quranPages).whenComplete(() {
      _loadOperation = null;
    });
  }

  Future<void> _loadQuran(
      {quranPages = QuranRepository.hafsPagesNumber}) async {
  ...
  Ayah? getAyahBySurahAndVerse(int surah, int verse) {
    try {
      return ayahs.firstWhere(
        (a) => a.surahNumber == surah && a.ayahNumber == verse,
      );
    } catch (_) {
      return null;
    }
